...
async function handleResult(id) {
    ...
    const [homeWinRate, awayWinRate] = scalePercentages(homeValueRate, awayValueRate); 
    ...
    const evHome = parseFloat((homeWinProb * (homeOdd - 1) - (1 - homeWinProb) * 1).toFixed(2));
    const evAway = parseFloat((awayWinProb * (awayOdd - 1) - (1 - awayWinProb) * 1).toFixed(2));

    const pbrHome = parseFloat((homeOdd / homeWinProb).toFixed(2));
    const pbrAway = parseFloat((awayOdd / awayWinProb).toFixed(2));

    const kellyHome = parseFloat(((homeWinProb * (homeOdd - 1) - (1 - homeWinProb)) / (homeOdd - 1)).toFixed(2));
    const kellyAway = parseFloat(((awayWinProb * (awayOdd - 1) - (1 - awayWinProb)) / (awayOdd - 1)).toFixed(2));
